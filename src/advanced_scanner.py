...
class AdvancedScanner:
    ...
    def scan_directory(self, directory: str, platforms: Optional[List[str]] = None) -> List[ScanFinding]:
    ...
    def _scan_general_security(self, directory: Path):
        ...
        text_extensions = ['.txt', '.md', '.json', '.yaml', '.yml', '.env', '.conf', '.cfg']
    ...
    def get_findings_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "platforms_scanned": list(set(finding.rule_id.split('-')[0] for finding in self.findings))
        }

    def export_findings(self, format: str = "json", output_file: Optional[str] = None) -> str:
