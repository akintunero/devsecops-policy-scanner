...
try:
    ...
    class SemverFallback:
        @staticmethod
        def parse_version_info(version_str: str) -> SimpleVersion:
            return SimpleVersion(version_str)

        class VersionInfo:
            @staticmethod
            def parse(version_str: str) -> SimpleVersion:
                return SimpleVersion(version_str)

    semver_lib = SemverFallback
...
class PolicyRegistry:
    ...
    def __init__(self, registry_path: str = "policy_registry"):
        ...
        self.policies: Dict[str, Dict[str, PolicyVersion]] = (
            {}
        )  # {name: {version: PolicyVersion}}
                ...
                with open(registry_file, "r") as f:
                    ...
                    for name, versions in data.items():
                        ...
                        for version, policy_data in versions.items():
                            ...
                            policy.created_at = datetime.fromisoformat(
                                policy_data["created_at"]
                            )
                            policy.updated_at = datetime.fromisoformat(
    ...
    def _save_registry(self):
        ...
        for name, versions in self.policies.items():
            data[name] = {
                version: policy.to_dict() for version, policy in versions.items()
            }
    ...
    def register_policy(
        ...
        # Validate version format (basic validation if semver not available)
        if SEMVER_AVAILABLE:
            try:
                semver_lib.parse_version_info(version)
            except ValueError:
                raise ValueError(
        ...
        if SEMVER_AVAILABLE:
            try:
                sorted_versions = sorted(
                    versions,
                    key=lambda v: semver_lib.VersionInfo.parse(v),
                    reverse=True,
                )
    ...
    def search_policies(
        ...
        for name, versions in self.policies.items():
            ...
            if query:
                ...
                if (
                    query_lower not in latest.name.lower()
                    and query_lower not in latest.description.lower()
                    and not any(query_lower in tag.lower() for tag in latest.tags)
                ):
                    continue
    ...
    def rate_policy(
        ...
        if policy.reviews:
            policy.rating = sum(r["rating"] for r in policy.reviews) / len(
                policy.reviews
            )
        else:
    ...
    def get_policy_dependencies(
        ...
        for dep in policy.dependencies:
            ...
            if dep_policy:
                ...
                dependencies.extend(
                    self.get_policy_dependencies(dep_name, dep_policy.version)
                )
