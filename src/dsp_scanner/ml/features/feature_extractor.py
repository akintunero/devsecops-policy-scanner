...
class SecurityFeatureExtractor:
    ...
    def _avg_code_complexity(self, result: ScanResult) -> float:
        ...
        code_lengths = [
            len(f.code_snippet or '')
            for f in result.findings
            if f.code_snippet
        ]
        return np.mean(code_lengths) if code_lengths else 0.0
    ...
    def _count_secrets(self, result: ScanResult) -> float:
        ...
        secret_keywords = ['secret', 'password', 'key', 'token', 'credential', 'api_key', 'private_key']
        return float(sum(
            1 for f in result.findings
            if any(kw in f.description.lower() or kw in f.title.lower() for kw in secret_keywords)
        ))
    
    def _count_vulnerabilities(self, result: ScanResult) -> float:
        """Count vulnerability findings."""
        vuln_keywords = ['vulnerability', 'cve', 'exploit', 'attack', 'injection', 'xss', 'sqli']
        return float(sum(
            1 for f in result.findings
            if any(kw in f.description.lower() or kw in f.title.lower() for kw in vuln_keywords)
        ))
    
    def _count_compliance_violations(self, result: ScanResult) -> float:
        """Count compliance violation findings."""
        compliance_keywords = ['compliance', 'policy', 'standard', 'requirement', 'violation']
        return float(sum(
            1 for f in result.findings
            if any(kw in f.description.lower() or kw in f.title.lower() for kw in compliance_keywords)
        ))
    
    def _count_dependency_issues(self, result: ScanResult) -> float:
        """Count dependency-related findings."""
        dep_keywords = ['dependency', 'package', 'library', 'module', 'npm', 'pip', 'maven']
        return float(sum(
            1 for f in result.findings
            if any(kw in f.description.lower() or kw in f.title.lower() for kw in dep_keywords)
        ))
    
    def _count_privilege_issues(self, result: ScanResult) -> float:
        """Count privilege escalation findings."""
        priv_keywords = ['privilege', 'permission', 'rbac', 'access control', 'escalation', 'sudo']
        return float(sum(
            1 for f in result.findings
            if any(kw in f.description.lower() or kw in f.title.lower() for kw in priv_keywords)
        ))
    
    def _count_network_exposure(self, result: ScanResult) -> float:
        """Count network exposure findings."""
        network_keywords = ['network', 'port', 'firewall', 'exposed', 'public', 'internet-facing']
        return float(sum(
            1 for f in result.findings
            if any(kw in f.description.lower() or kw in f.title.lower() for kw in network_keywords)
        ))
    ...
    def _count_authentication_issues(self, result: ScanResult) -> float:
        ...
        return float(sum(
            1 for f in result.findings
            if any(kw in f.description.lower() or kw in f.title.lower() for kw in auth_keywords)
        ))
    ...
    def _count_encryption_issues(self, result: ScanResult) -> float:
        ...
        return float(sum(
            1 for f in result.findings
            if any(kw in f.description.lower() or kw in f.title.lower() for kw in enc_keywords)
        ))
    ...
    def _finding_diversity_score(self, result: ScanResult) -> float:
        ...
        return float(len(unique_types) / len(result.findings)) if result.findings else 0.0
