...
class RealTimeMonitor:
    ...
    async def notify_scan_completed(self, scan_id: str, result: ScanResult):
        ...
        if result.ai_analysis and result.ai_analysis.risk_predictions:
            risk_score = result.ai_analysis.risk_predictions[0].get(
                "predicted_risk_score", 0
            )
            # Update rolling average
            current_avg = self.metrics["risk_score_avg"]
            total_scans = self.metrics["total_scans"]
            self.metrics["risk_score_avg"] = (
                current_avg * (total_scans - 1) + risk_score
            ) / total_scans
        ...
        event = MonitoringEvent(
            ...
            data={
                ...
                "risk_score": (
                    result.ai_analysis.risk_predictions[0].get(
                        "predicted_risk_score", 0
                    )
    ...
    def get_dashboard_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_events": [asdict(event) for event in self.event_history[-50:]],
