...
from dsp_scanner.federated_learning.federated_engine import (
...
from dsp_scanner.monitoring.realtime_monitor import RealTimeMonitor
...
class UnifiedSecurityPlatform:
    ...
    def __init__(
        ...
        self.federated_learning = (
            FederatedLearningEngine() if enable_federated_learning else None
        )
    ...
    async def scan_with_full_analysis(
        ...
        # Notify monitoring
        if self.monitor:
            await self.monitor.notify_scan_started(
                scan_id,
                {"path": path, "frameworks": [f.value for f in (frameworks or [])]},
            )
        ...
        if self.compliance and frameworks:
            compliance_reports = self.compliance.assess_compliance(
        ...
        if self.federated_learning and enable_federated_update:
            try:
                ...
                participant_id = (
                    "default_participant"  # In production, use actual participant ID
                )
                self.federated_learning.prepare_local_update(
        ...
        return {
            ...
            "compliance_reports": {
                framework.value: report.__dict__
                for framework, report in compliance_reports.items()
            },
            "monitoring": {
                "dashboard_data": (
                    self.monitor.get_dashboard_data() if self.monitor else None
                ),
                "event_history": (
                    self.monitor.get_event_history(limit=50) if self.monitor else []
                ),
            },
            "marketplace": {
                "available_policies": (
                    len(self.marketplace.list_policies()) if self.marketplace else 0
                )
            },
            "federated_learning": {
                "participant_stats": (
                    self.federated_learning.get_participant_stats()
    ...
    async def install_policy_from_marketplace(
        self, policy_name: str, version: Optional[str] = None
    ) -> PolicyVersion:
    ...
    async def run_federated_round(self, updates: List[FederatedModelUpdate]):
    ...
    def get_platform_status(self) -> Dict[str, Any]:
        ...
        return {
            "monitoring": {
                ...
                "connected_clients": (
                    len(self.monitor.connected_clients) if self.monitor else 0
                ),
            ...
            "compliance": {
                ...
                "frameworks_supported": (
                    len(self.compliance.frameworks) if self.compliance else 0
                ),
            },
            "federated_learning": {
                "enabled": self.federated_learning is not None,
                "participants": (
                    len(self.federated_learning.participants)
                    if self.federated_learning
                    else 0
                ),
                "rounds_completed": (
                    len(self.federated_learning.rounds)
                    if self.federated_learning
                    else 0
                ),
            },
