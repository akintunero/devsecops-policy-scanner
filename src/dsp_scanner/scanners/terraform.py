...
class TerraformScanner:
    ...
    async def scan(
        ...
        if hcl2 is None:
            raise ImportError(
                "Terraform scanning requires the 'python-hcl2' package (import name: hcl2)."
            )
        ...
        scan_result.platforms.append("terraform")
    ...
    def _find_terraform_files(self, path: Path) -> List[Path]:
        ...
        if path.is_file() and self._is_terraform_file(path):
            ...
            tf_files.extend(f for f in path.rglob("*") if self._is_terraform_file(f))

        logger.info(f"Found {len(tf_files)} Terraform file(s)")
        return tf_files

    async def _scan_terraform_file(
        self, tf_file: Path, policies: Optional[List[Policy]] = None
    ) -> List[Finding]:
        """
        ...
        try:
            with tf_file.open() as f:
                ...
                # Scan resources (python-hcl2 returns a list of blocks)
                ...
                provider_blocks = config.get("provider", [])
                ...
                if providers:
                    findings.extend(self._check_provider_security(providers, tf_file))

                # Check variables
                variable_blocks = config.get("variable", [])
                ...
                if variables:
                    findings.extend(self._check_variable_security(variables, tf_file))

                # Apply custom policies if provided
                if policies:
                    policy_findings = await self._apply_policies(
                        config, tf_file, policies
                    )
    ...
    def _check_resource_security(
        ...
        if resource_type.startswith("aws_"):
            findings.extend(
                self._check_aws_resource(
                    resource_type, resource_name, resource, tf_file
                )
            )
    ...
    def _check_provider_security(
        self, providers: Dict[str, Any], tf_file: Path
    ) -> List[Finding]:
    ...
    def _check_variable_security(
        self, variables: Dict[str, Any], tf_file: Path
    ) -> List[Finding]:
    ...
    async def _apply_policies(
        self, config: Dict[str, Any], tf_file: Path, policies: List[Policy]
    ) -> List[Finding]:
                ...
                if result.get("violations"):
                    findings.extend(
                        self._convert_policy_violations(
                            result["violations"], tf_file, policy
                        )
                    )
            except Exception as e:
    ...
    def _convert_policy_violations(
        self, violations: List[Dict[str, Any]], tf_file: Path, policy: Policy
    ) -> List[Finding]:
    ...
    def _is_terraform_file(path: Path) -> bool:
        """Check if a path looks like a Terraform configuration file."""
