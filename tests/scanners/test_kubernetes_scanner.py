                    ...
                    "containers": [
                        {
                            ...
                            "volumeMounts": [
                                {"name": "host-path", "mountPath": "/host"}
                            ],
                        }
                    ],
                    "volumes": [{"name": "host-path", "hostPath": {"path": "/"}}],
                }
            },
        },
    }
...
async def test_scan_network_policy(scanner, tmp_path):
    ...
    policy = {
        ...
        "spec": {"podSelector": {}, "ingress": [{}]},  # Empty rule allows all
    }
...
async def test_scan_secure_pod(scanner, tmp_path):
    ...
    pod = {
        "apiVersion": "v1",
        "kind": "Pod",
        "metadata": {"name": "secure-pod", "namespace": "prod"},
        "spec": {
            "securityContext": {
                "runAsNonRoot": True,
                "runAsUser": 1000,
...
def test_is_kubernetes_manifest():
    ...
    valid_content = {"apiVersion": "v1", "kind": "Pod", "metadata": {"name": "test"}}

    invalid_content = {"name": "test", "description": "Not a K8s manifest"}
...
async def test_scan_container_images(scanner, tmp_path):
    ...
    assert any(
        f.id == "K8S010" and "app:latest" in f.description for f in result.findings
    )
    assert any(
        f.id == "K8S010" and "busybox:latest" in f.description for f in result.findings
    )
