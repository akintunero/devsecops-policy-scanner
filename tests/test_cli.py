...
def mock_scanner():
    ...
    scanner = Mock()
    scanner.scan_path = AsyncMock(return_value=ScanResult())
    scanner.compliance_frameworks = []
    scanner.severity_threshold = "medium"
    return scanner
...
def test_help():
    ...
    # Typer/Click formatting varies by version; accept either header style.
...
async def test_scan_with_platform_filter(tmp_path, mock_scanner):
    ...
    with patch("dsp_scanner.cli.Scanner", return_value=mock_scanner):
        result = runner.invoke(
            app,
            ["scan", str(tmp_path), "--platform", "docker", "--platform", "kubernetes"],
        )
...
async def test_scan_with_compliance_frameworks(tmp_path, mock_scanner):
    ...
    with patch("dsp_scanner.cli.Scanner", return_value=mock_scanner):
        result = runner.invoke(
            app, ["scan", str(tmp_path), "--compliance", "cis", "--compliance", "nist"]
        )
...
async def test_scan_with_severity_threshold(tmp_path, mock_scanner):
    ...
    with patch("dsp_scanner.cli.Scanner", return_value=mock_scanner):
        result = runner.invoke(app, ["scan", str(tmp_path), "--severity", "high"])
...
async def test_scan_with_output_file(tmp_path, mock_scanner, sample_findings):
    ...
    with patch("dsp_scanner.cli.Scanner", return_value=mock_scanner):
        result = runner.invoke(
            app,
            ["scan", str(tmp_path), "--output", str(output_file), "--format", "json"],
        )
...
def test_scan_display_format(tmp_path, mock_scanner, sample_findings):
    ...
    # Rich renders markup to terminal control codes; assert on rendered text.
    assert "Detailed Findings" in result.stdout
    assert "HIGH" in result.stdout
    assert "MEDIUM" in result.stdout
    assert "│" in result.stdout or "┏" in result.stdout  # table borders vary by env
