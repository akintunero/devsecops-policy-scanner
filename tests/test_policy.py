...
from dsp_scanner.core.policy import (
    Policy,
...
@pytest.fixture
def sample_policy(sample_rego_policy):
...
async def test_policy_evaluation(sample_policy):
    ...
    input_data = {"kind": "Deployment", "spec": {"template": {"spec": {}}}}
...
async def test_policy_set_evaluation(sample_policy):
    ...
    policy_set = PolicySet(
        name="test_set", description="Test policy set", policies=[sample_policy]
    )
...
@pytest.mark.asyncio
async def test_policy_manager_builtin_policies(tmp_path, sample_rego_policy):
    ...
    await manager.load_builtin_policies(base_dir=tmp_path)
...
async def test_policy_manager_evaluation(sample_policy):
    ...
    results = await manager.evaluate_policies(
        input_data, policy_sets=None, platform="kubernetes"
    )
...
def test_policy_set_management():
    ...
    policy_set = PolicySet(name="test_set", description="Test policy set")
